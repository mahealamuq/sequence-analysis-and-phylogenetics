        ...
        for i in range(len(self.seqs)):
            for j in range(i + 1, len(self.seqs)):
                ...
                if measure == 'fractional':
                    ...
                    # Poisson corrected distance (Zvelebil & Baum p270):
                    # d = -ln(1 - p)
                    # Corrects for the probability that a site has changed more than once.
                    # Note: undefined (math error) when p >= 1; add guard.
                    if p >= 1.0:
                        dist = float('inf')  # Saturated: sequences too divergent to correct
                    else:
                        dist = -math.log(1 - p)  # ADDED: Poisson correction formula
                elif measure == 'gamma':
                    # Gamma corrected distance (Zvelebil & Baum p271):
                    # d = a * ((1 - p)^(-1/a) - 1)
                    # The shape parameter 'a' accounts for rate variation across sites.
                    # When a → ∞ this converges to the Poisson distance.
                    if p >= 1.0:
                        dist = float('inf')  # Saturated
                    else:
                        dist = a * ((1 - p) ** (-1.0 / a) - 1)  # ADDED: Gamma correction formula
...
def getSequence(entryId, dbName = 'uniprotkb', alphabet = Protein_Alphabet, format = 'fasta', debug: bool = True):
    ...
    dbName: name of database e.g. 'uniprotkb' or 'pdb' or 'refseqn'; 
    see http://www.ebi.ac.uk/Tools/dbfetch/dbfetch/dbfetch.databases for available databases
    format: file format specific to database e.g. 'fasta' or 'uniprot' 
    for uniprotkb (see http://www.ebi.ac.uk/Tools/dbfetch/dbfetch/dbfetch.databases)
